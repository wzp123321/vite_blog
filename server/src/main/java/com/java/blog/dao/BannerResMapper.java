...
public interface BannerResMapper {
    ...
    @Update({
            ...
            "set title= #{title,jdbcType=VARCHAR},image_url= #{imageUrl,jdbcType=VARCHAR},article_id= #{articleId,jdbcType=VARCHAR},state= #{state,jdbcType=INTEGER}",
            "where id = #{id,jdbcType=INTEGER}"
    })
    int getBannerVoUpdate(@Param("id") int id, @Param("title") String title, @Param("imageUrl") String imageUrl, @Param("articleId") String articleId, @Param("state") Integer state);
    ...
    @Delete({
            "delete from zpwan_blog_banners",
            "where id = #{id,jdbcType=INTEGER}"
    })
    int getBannerVoDelete(@Param("id") Integer id);
}
