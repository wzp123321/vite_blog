...
public class DictionaryService {
     ...
     * @return
     */
    public ModelMap getDictionaryList(Integer type, Integer parentId, int page, int limit) {
        int skip = page > 1 ? (page - 1) * limit : 0;
        int count = dictionaryResMapper.getDictionaryVoCount(type, parentId);
        List<DictionaryVo> dictionaryVos = dictionaryResMapper.getDictionaryList(type, parentId, skip, limit);
        ModelMap modelMap = new ModelMap();
        modelMap.put("list", dictionaryVos);
        modelMap.put("total", count);
        logger.info("查询字典列表----" + type);
        return modelMap;
    }
}
